...
func NewTestLogger(tb testing.TB, options ...Option) (*wlogging.WswLogger, *Recorder) {
	enabler := zap.LevelEnablerFunc(func(l zapcore.Level) bool {
		return zapcore.DebugLevel.Enabled(l)
	})

	formatters, err := wenc.ParseFormat(DefaultFormat)
	if err != nil {
		tb.Fatalf("failed to parse format %s: %s", DefaultFormat, err)
	}
	encoder := wenc.NewFormatEncoder(formatters...)
	if err != nil {
	...
	return wlogging.NewWswLogger(zl, zap.AddCaller()), recorder
}
